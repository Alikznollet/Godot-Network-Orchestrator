...
func get_updates_as_dicts() -> Array[Dictionary]:
	...
	for link_id in updates:
		...
		if not upd.unlinked: 
			var ls: LinkedState = linked_states.get(link_id)
...
func add_update(link_id: int, data: LinkedData, unlinked: bool = false) -> void:
	...
	upd.unlinked = unlinked
	upd.add_data(data)

## Apply all of the incoming LinkedUpdate objects.
func apply_updates(update_dicts: Array[Dictionary]) -> void:
	for update_dict in update_dicts:
		var lu: LinkedUpdate = LinkedUpdate.from_dictionary(update_dict)
		apply_update(lu)

func apply_update(lu: LinkedUpdate) -> void:
	# First handle whether the update was unlinked or not.
	if lu.unlinked:
		var ls: LinkedState = linked_states.get(lu.link_id)
		if ls:
			linked_states.erase(ls.id)
			ls.unlink()
		return
	...
	var ls: LinkedState
	if linked_states.has(lu.link_id):
		ls = linked_states.get(lu.link_id)
	else:
		ls = LinkedStateRegistry.get_script_from_id(lu.link_type).new()
		ls.id = lu.link_id
		linked_states[lu.link_id] = ls
		ls.local_state_change.connect(local_change)
		ls.external_state_change.connect(external_change)
		ls.event_state_change.connect(event_change)
